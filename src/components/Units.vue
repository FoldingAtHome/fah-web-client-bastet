<template>
  <div class="view">
    <h2>
      Work Units
      <button type="button" class="btn pauseBtn" :class="[!areAllRunning ? 'btn-success' : 'btn-warning']"
              @click="pauseAll">
    ...
    <table class="table">
      ...
      <tbody>
        <tr v-for="(unit, index) in units" :key="index">
          ...
          <td>{{ unit.state }}</td>
          <td>
            <div v-if="unit.state == 'RUN'" class="progress">
              <div class="progress-bar progress-bar-striped" role="progressbar"
                   :class="[unit.paused ? 'bg-secondary' : 'progress-bar-animated bg-success']"
                   :style="{ width: unit.progress * 100 + '%' }" aria-valuenow="50" aria-valuemin="0"
                   aria-valuemax="100">
